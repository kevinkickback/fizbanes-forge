...
export class SpellSelectionModal {
    ...
    async show() {
        ...
        if (!this.className) {
            // If no class specified, try to use primary class
            const primaryClass = character.getPrimaryClass();
            if (primaryClass?.name) {
                this.className = primaryClass.name;
            } else {
                // Allow adding spells even without a class (e.g., for racial traits)
                // Use a generic "Racial" or "Other" category
                this.className = 'Racial';
            }
        }

        // Auto-enable "ignore class restrictions" if no spellcasting class available
        const spellcastingClasses = ['Bard', 'Cleric', 'Druid', 'Paladin', 'Ranger', 'Sorcerer', 'Warlock', 'Wizard'];
        const hasSpellcastingClass = spellcastingClasses.includes(this.className);
