...
async function buildLocalIndexManifest(rootDir) {
	...
	for (const indexPath of ENUMERATION_INDEX_FILES) {
		...
		try {
			...
			indexErrors.push({
				indexPath,
				error: error?.message || 'Invalid index JSON',
			});
			MainLogger.warn(
				'DataFolderManager',
				'Failed to read local index for manifest',
				{
					indexPath,
					error: error?.message,
				},
...
export async function validateLocalDataFolder(folderPath) {
	try {
		...
		const { manifest: indexedManifest, indexErrors } =
			await buildLocalIndexManifest(folderPath);
...
export async function buildDataManifest(remoteUrl) {
	...
	for (const indexPath of ENUMERATION_INDEX_FILES) {
		...
		try {
			...
			MainLogger.warn(
				'DataFolderManager',
				'Failed to expand manifest from index',
				{
					indexPath,
					error: error?.message,
				},
