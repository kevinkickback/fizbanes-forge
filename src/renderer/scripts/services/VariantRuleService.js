...
Logger.debug('VariantRuleService', 'Already initialized');
return true;
}

Logger.info('VariantRuleService', 'Initializing variant rule data');

try {
this._variantRuleData = await DataLoader.loadVariantRules();
Logger.info('VariantRuleService', 'Variant rules loaded successfully', {
...
Logger.error('VariantRuleService', 'Failed to initialize variant rule data', error);
return false;
}
}
