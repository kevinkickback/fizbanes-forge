...
export class StatBlockRenderer {
    ...
    static renderSpell(spell) {
        ...
        // Title and level
        const levelText = spell.level === 0 ? 'Cantrip' : `${StatBlockRenderer._getOrdinal(spell.level)}-level`;
        const schoolText = spell.school ? ` ${StatBlockRenderer._getSchoolName(spell.school)}` : '';
        ...
        if (spell.time || spell.range || spell.components || spell.duration) {
            ...
            if (spell.range) {
                const rangeText = StatBlockRenderer._getRangeText(spell.range);
                html += `<strong>Range:</strong><span>${rangeText}</span>`;
            }

            if (spell.components) {
                const compText = StatBlockRenderer._getComponentsText(spell.components);
                html += `<strong>Components:</strong><span>${compText}</span>`;
            }

            if (spell.duration?.[0]) {
                const durText = StatBlockRenderer._getDurationText(spell.duration[0]);
        ...
        // Description
        html += StatBlockRenderer._renderEntries(spell.entries);

        // At higher levels
        if (spell.entriesHigherLevel && spell.entriesHigherLevel.length > 0) {
            html += '<div class="tooltip-section">';
            html += StatBlockRenderer._renderEntries(spell.entriesHigherLevel);
            html += '</div>';
        }

        // Source
        html += StatBlockRenderer._renderSource(spell);
        html += '</div>';
    ...
    static renderItem(item) {
        ...
        // Description
        if (item.entries) {
            html += StatBlockRenderer._renderEntries(item.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(item);
        html += '</div>';
    ...
    static renderRace(race) {
        ...
        if (race.size) {
            const sizeText = Array.isArray(race.size) ? race.size.join(', ') : race.size;
        ...
        if (race.ability && Array.isArray(race.ability)) {
            const abilities = StatBlockRenderer._getAbilityScoreText(race.ability);
            if (abilities) {
        ...
        // Description
        if (race.entries) {
            html += StatBlockRenderer._renderEntries(race.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(race);
        html += '</div>';
    ...
    static renderClass(cls) {
        ...
        if (cls.entries && cls.entries.length > 0) {
            ...
            if (typeof firstEntry === 'string') {
                const shortDesc = firstEntry.length > 150 ? `${firstEntry.substring(0, 150)}...` : firstEntry;
                html += getStringRenderer().render(shortDesc);
            } else if (firstEntry.entries) {
                const text = firstEntry.entries[0];
                if (typeof text === 'string') {
                    const shortDesc = text.length > 150 ? `${text.substring(0, 150)}...` : text;
                    html += getStringRenderer().render(shortDesc);
                }
            }
            html += '</div>';
        ...
        // Source
        html += StatBlockRenderer._renderSource(cls);
        html += '</div>';
    ...
    static renderFeat(feat) {
        ...
        if (feat.prerequisite) {
            html += '<div class="tooltip-metadata">';
            html += `<strong>Prerequisite:</strong> ${StatBlockRenderer._formatPrerequisite(feat.prerequisite)}<br>`;
            html += '</div>';
        }

        // Description
        if (feat.entries) {
            html += StatBlockRenderer._renderEntries(feat.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(feat);
        html += '</div>';
     ...
     * @returns {string} HTML
     */
    static renderBackground(background) {
        ...
        if (background.skillProficiencies) {
            ...
            const skills = [];
            for (const [skill, value] of Object.entries(background.skillProficiencies)) {
                if (value === true) {
                    skills.push(skill);
                }
            }
        ...
        if (background.entries) {
            html += StatBlockRenderer._renderEntries(background.entries, 3); // Limit to 3 entries for backgrounds
        }

        // Source
        html += StatBlockRenderer._renderSource(background);
        html += '</div>';
    ...
    static renderCondition(condition) {
        ...
        if (condition.entries) {
            html += StatBlockRenderer._renderEntries(condition.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(condition);
        html += '</div>';
    ...
    static renderSkill(skill) {
        ...
        // Description
        if (skill.entries) {
            html += StatBlockRenderer._renderEntries(skill.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(skill);
        html += '</div>';
    ...
    static renderAction(action) {
        ...
        if (action.time && Array.isArray(action.time)) {
            html += '<div class="tooltip-metadata">';
            const timeStr = action.time.map(t => `${t.number || 1} ${t.unit}`).join(', ');
        ...
        // Description
        if (action.entries) {
            html += StatBlockRenderer._renderEntries(action.entries);
        }

        // Source
        html += StatBlockRenderer._renderSource(action);
        html += '</div>';
     ...
     * @private
     */
    static _renderEntries(entries, maxEntries = 5) {
        if (!entries || !Array.isArray(entries)) return '';

        let html = '<div class="tooltip-description">';
        const renderer = getStringRenderer();

        for (let i = 0; i < Math.min(entries.length, maxEntries); i++) {
            const entry = entries[i];

            if (typeof entry === 'string') {
                html += `<p>${renderer.render(entry)}</p>`;
                ...
                for (let j = 0; j < Math.min(entry.items.length, 5); j++) {
                    ...
                    if (typeof item === 'string') {
                        ...
                        html += `<li>${nameText}${renderer.render(item.entry)}</li>`;
                    }
                }
                html += '</ul>';
     ...
     * @private
     */
    static _getComponentsText(components) {
        const parts = [];
     ...
     * @private
     */
    static _getDurationText(duration) {
     ...
     * @private
     */
    static _getAbilityScoreText(abilities) {
        const parts = [];
     ...
     * @private
     */
    static _formatPrerequisite(prerequisite) {
