...
test.describe('Preload Hardened API', () => {
    test('exposes whitelisted domains and methods', async () => {
        ...
        const hasFFDebug = await mainWindow.evaluate(() => typeof window.FF_DEBUG !== 'undefined');
        expect(hasFFDebug).toBeTruthy();

        const hasApp = await mainWindow.evaluate(() => typeof window.app !== 'undefined');
        const hasData = await mainWindow.evaluate(() => typeof window.data !== 'undefined');
        const hasCharacterStorage = await mainWindow.evaluate(() => typeof window.characterStorage !== 'undefined');
        expect(hasApp).toBeTruthy();
        expect(hasData).toBeTruthy();
        expect(hasCharacterStorage).toBeTruthy();

        // Ensure generic invoke bridges are not present
        const hasGenericInvoke = await mainWindow.evaluate(() => !!(window.electron?.invoke));
    ...
    test('can generate UUID and list characters', async () => {
        ...
        const uuidResult = await mainWindow.evaluate(() => window.characterStorage.generateUUID());
    ...
    test('can load app data JSON via data domain', async () => {
        ...
        const skillsResult = await mainWindow.evaluate(() => window.data.loadJSON('data/skills.json'));
