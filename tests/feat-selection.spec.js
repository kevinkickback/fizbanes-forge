...
test.describe('Feat Selection', () => {
	...
	test('feat selection modal opens and allows selection for Variant Human', async () => {
		...
		try {
			...
			const firstFeatName = await firstFeat
				.locator('strong')
				.first()
				.textContent();
			...
			const firstFeatSelected = await firstFeat
				.locator('.feat-selected-indicator')
				.isVisible();
			console.log(
				`First feat selected indicator visible: ${firstFeatSelected}`,
			);
			...
			const secondFeatName = await secondFeat
				.locator('strong')
				.first()
				.textContent();
			...
			if (isDisabled === 'true') {
				...
				const secondFeatSelected = await secondFeat
					.locator('.feat-selected-indicator')
					.isVisible();
				console.log(
					`Second feat was clickable, now selected: ${secondFeatSelected}`,
				);
			}
			...
			console.log(
				`Feat list content: "${featListHtml?.substring(0, 200) || 'EMPTY'}"`,
			);
	...
	test('feat selection modal pre-selects saved feats', async () => {
		...
		try {
			...
			const subraceDropdown = page.locator(
				'[id$="Subrace"] select.form-select',
			);
			...
			// Wait for modal
			await page.waitForSelector('.modal-overlay .feat-list', {
				timeout: 10000,
			});
			console.log('✓ Modal appeared');
