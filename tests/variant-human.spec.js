...
test.describe('Variant Human Loading', () => {
    test('creates new character with Variant Human, saves, and verifies persistence', async () => {
        ...
        try {
            ...
            const characterCard = page.locator('.character-card', { hasText: testCharacterName }).first();
            ...
            const selectedSubrace = await subraceSelect.locator('option:checked').textContent();
            ...
            page = electronApp.windows().find((win) => !win.url().startsWith('devtools://'));
            if (!page) {
                page = await electronApp.waitForEvent('window', (win) => !win.url().startsWith('devtools://'));
            }
            ...
            const reloadedCard = page.locator('.character-card', { hasText: testCharacterName }).first();
            ...
            const reloadedSubrace = await reloadedSubraceSelect.locator('option:checked').textContent();
            ...
            const loadingLogs = consoleLogs.filter(
                (log) => log.text.includes('Loading saved subrace') && log.text.includes('Variant'),
            );
            ...
            await page.waitForSelector('button.nav-link[data-page="home"]', { timeout: 15000 });
            ...
            const deleteCard = page.locator('.character-card', { hasText: testCharacterName }).first();
            ...
            const confirmButton = page.locator('#confirmDeleteBtn, .btn-danger').filter({ hasText: /delete|confirm/i }).first();
            ...
            const deletedCard = page.locator('.character-card', { hasText: testCharacterName });
            ...
            if (characterCreated && electronApp) {
                try {
                    ...
                    const page = electronApp.windows().find((win) => !win.url().startsWith('devtools://'));
                    if (page) {
