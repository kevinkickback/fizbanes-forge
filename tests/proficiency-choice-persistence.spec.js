...
async function selectRaceAndSkillProficiencies(page) {
	...
	const skillCheckboxes = page.locator(
...
test.describe('Proficiency Choice Persistence', () => {
	test('persists skill selections across app restart after race selection', async () => {
		...
		try {
			...
			await secondPage.waitForSelector('button.nav-link[data-page="home"]', {
				timeout: 15000,
			});
			await secondPage.click('button.nav-link[data-page="home"]');
			await secondPage.waitForSelector('[data-current-page="home"]', {
				timeout: 30000,
			});
			await secondPage.waitForTimeout(1000);

			const deleteCard = secondPage
			...
			const confirmButton = secondPage
			...
			await secondPage.waitForTimeout(1000);
			const deletedCard = secondPage.locator('.character-card', {
