...
test.describe('Race Change Dropdown Reset', () => {
    test('resets ability choice dropdowns when changing race', async () => {
        ...
        try {
            ...
            const abilitySelectsAfterChange = await page.locator('.ability-choice-select').count();
            ...
            await expect(page.locator('.ability-choice-select')).toHaveCount(2, { timeout: 15000 });
            ...
            console.log('✓ Ability choice dropdowns properly reset when changing race');
        } finally {
            if (electronApp) {
    ...
    test('resets ability choice dropdowns when changing subrace', async () => {
        ...
        try {
            ...
            console.log('✓ Ability choice dropdowns properly reset when changing subrace');
        } finally {
            if (electronApp) {
                await electronApp.close();
            }
