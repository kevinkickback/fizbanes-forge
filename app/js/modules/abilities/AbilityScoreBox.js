...
class AbilityScoreBoxView {
     ...
     */
    renderAllAbilityScores(isStandardArray, isPointBuy, isCustom, onStandardArrayChange) {
        try {
            // Process each ability score
            for (const ability of abilityScoreService.getAllAbilities()) {
                this.renderAbilityScoreBox(ability, isStandardArray, isPointBuy, isCustom, onStandardArrayChange);
            }
        } catch (error) {
     ...
     * @param {Function} onStandardArrayChange - Callback for standard array changes
     */
    renderAbilityScoreBox(ability, isStandardArray, isPointBuy, isCustom, onStandardArrayChange) {
        ...
        box.querySelector('.modifier').textContent = abilityScoreService.getModifierString(ability);
        ...
        if (isStandardArray) {
            methodControlsView.renderStandardArrayControls(controlsContainer, ability, baseScore, onStandardArrayChange);
        } else if (isPointBuy) {
            methodControlsView.renderPointBuyControls(controlsContainer, ability, baseScore);
        } else if (isCustom) {
            methodControlsView.renderCustomControls(controlsContainer, ability, baseScore);
        }
        ...
        try {
            ...
            for (const [ability, score] of Object.entries(scores)) {
                ...
                if (box) {
                    ...
                    if (modifierElement) {
                        modifierElement.textContent = abilityScoreService.getModifierString(ability);
                    }
