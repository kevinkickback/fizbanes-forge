        ...
        if (race?.ability) {
            for (const abilityEntry of race.ability) {
                if (abilityEntry.choose) {
                    ...
                    improvements.push(`Increase ${count} ability score${count > 1 ? 's' : ''} by ${amount}`);
                } else {
                    // Fixed improvements
    ...
    _formatLanguages(race) {
        ...
        for (const langEntry of race.languageProficiencies) {
            if (langEntry.anyStandard) {
                languages.push(`Choose ${langEntry.anyStandard} standard language${langEntry.anyStandard > 1 ? 's' : ''}`);
                        ...
                        const processedEntries = await Promise.all(
                            trait.entries.map(entry => {
                                if (typeof entry === 'string') {
                                    ...
                                    // Handle list entries
                                    return Promise.all(entry.items.map(item =>
                                        textProcessor.processString(typeof item === 'string' ? item : '')
