...
export class ProficiencySelectionView {
    ...
    toggleOptionalProficiency(profItem, character) {
        ...
        try {
            if (profType === 'skills') {
                changed = this._toggleSkillProficiency(profItem, proficiency, character);
            } else if (profType === 'languages') {
                changed = this._toggleLanguageProficiency(profItem, proficiency, character);
                ...
                changed = this._toggleSimpleProficiency(profItem, proficiency, profType, character);
            }
        } catch (error) {
    ...
    _toggleSkillProficiency(profItem, proficiency, character) {
        ...
        const raceOptions = skillOptions.race?.options?.map(o => o.toLowerCase()) || [];
        const classOptions = skillOptions.class?.options?.map(o => o.toLowerCase()) || [];
        const backgroundOptions = skillOptions.background?.options?.map(o => o.toLowerCase()) || [];
        ...
        if (profItem.classList.contains('selected') || profItem.classList.contains('optional-selected')) {
            return this._deselectSkill(profItem, proficiency, normalizedProf, skillOptions, raceSelected, classSelected, backgroundSelected, combinedSelected);
        }
        // Handle selection
        else {
            return this._selectSkill(profItem, proficiency, normalizedProf, skillOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny);
        }
    }
    ...
    _deselectSkill(profItem, _proficiency, normalizedProf, skillOptions, raceSelected, classSelected, backgroundSelected, combinedSelected) {
        ...
        if (raceSelected.some(p => p.toLowerCase() === normalizedProf)) {
            skillOptions.race.selected = raceSelected.filter(p => p.toLowerCase() !== normalizedProf);
            removedFromSource = true;
        }
        if (classSelected.some(p => p.toLowerCase() === normalizedProf)) {
            skillOptions.class.selected = classSelected.filter(p => p.toLowerCase() !== normalizedProf);
            removedFromSource = true;
        }
        if (backgroundSelected.some(p => p.toLowerCase() === normalizedProf)) {
            skillOptions.background.selected = backgroundSelected.filter(p => p.toLowerCase() !== normalizedProf);
            removedFromSource = true;
        }
        ...
        skillOptions.selected = combinedSelected.filter(p => p.toLowerCase() !== normalizedProf);
     ...
     * @private
     */
    _selectSkill(profItem, proficiency, normalizedProf, skillOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny) {
        const raceSelected = skillOptions.race?.selected || [];
        const classSelected = skillOptions.class?.selected || [];
        const backgroundSelected = skillOptions.background?.selected || [];
        ...
        if (assignedToSource) {
            if (!skillOptions.selected.some(p => p.toLowerCase() === normalizedProf)) {
    ...
    _toggleLanguageProficiency(profItem, proficiency, character) {
        ...
        // Handle deselection
        if (profItem.classList.contains('selected') || profItem.classList.contains('optional-selected')) {
            return this._deselectLanguage(profItem, proficiency, normalizedProf, languageOptions, raceSelected, classSelected, backgroundSelected, combinedSelected);
        }
        // Handle selection
        else {
            return this._selectLanguage(profItem, proficiency, normalizedProf, languageOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny);
        }
    }
     ...
     * @private
     */
    _selectLanguage(profItem, proficiency, normalizedProf, languageOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny) {
        const raceSelected = languageOptions.race?.selected || [];
        const classSelected = languageOptions.class?.selected || [];
        const backgroundSelected = languageOptions.background?.selected || [];
    ...
    _toggleToolProficiency(profItem, proficiency, character) {
        ...
        // Handle deselection
        if (profItem.classList.contains('selected') || profItem.classList.contains('optional-selected')) {
            return this._deselectTool(profItem, proficiency, normalizedProf, toolOptions, raceSelected, classSelected, backgroundSelected, combinedSelected);
        }
        // Handle selection
        else {
            return this._selectTool(profItem, proficiency, normalizedProf, toolOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny);
        }
    }
     ...
     * @private
     */
    _selectTool(profItem, proficiency, normalizedProf, toolOptions, isRaceOption, isClassOption, isBackgroundOption, raceAllowsAny, classAllowsAny, backgroundAllowsAny) {
        const raceSelected = toolOptions.race?.selected || [];
        const classSelected = toolOptions.class?.selected || [];
        const backgroundSelected = toolOptions.background?.selected || [];
    ...
    _toggleSimpleProficiency(profItem, proficiency, profType, character) {
        const selectedProfs = character.optionalProficiencies[profType]?.selected || [];
        const allowedCount = character.optionalProficiencies[profType]?.allowed || 0;

        if (profItem.classList.contains('selected') || profItem.classList.contains('optional-selected')) {
            // Remove proficiency from selection
            character.optionalProficiencies[profType].selected = selectedProfs.filter(p => p !== proficiency);
            this._updateItemUIForDeselection(profItem);
            return true;
        } else {
