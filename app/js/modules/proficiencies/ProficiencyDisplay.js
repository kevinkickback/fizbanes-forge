...
export class ProficiencyDisplayView {
     ...
     */
    renderContainers(containers, character, availableOptionsMap, defaultProficiencies, isGrantedBySource, isProficiencyAvailable, getIconForType, proficiencyManager) {
        if (!character) return;
        ...
        for (const type of this._proficiencyTypes) {
            ...
            const selectedCount = character?.optionalProficiencies?.[type]?.selected?.length || 0;
            const combinedSlotsAvailable = optionalAllowed > 0 && selectedCount < optionalAllowed;

            // Handle selection counter in section header
            this._updateSelectionCounter(container, type, selectedCount, optionalAllowed);
     ...
     * @private
     */
    _buildContainerHtml(type, availableOptions, character, defaultProficiencies, isGrantedBySource, isProficiencyAvailable, combinedSlotsAvailable, getIconForType, proficiencyManager) {
        let containerHtml = '';

        for (const item of availableOptions) {
            // Check if this proficiency is already selected (part of combined list)
            const isOptionallySelected = character?.optionalProficiencies?.[type]?.selected?.includes(item) || false;
            ...
            if (cssClasses.includes('selectable') && (type === 'skills' || type === 'languages')) {
    ...
    _addSourceSpecificClasses(cssClasses, type, item, character) {
        const raceOptions = character.optionalProficiencies?.[type]?.race?.options || [];
        const classOptions = character.optionalProficiencies?.[type]?.class?.options || [];
        const backgroundOptions = character.optionalProficiencies?.[type]?.background?.options || [];
     ...
     * @private
     */
    _buildItemHtml(cssClasses, item, type, isOptionallySelected, getIconForType, proficiencyManager) {
        ...
        return `<div class="${cssClasses.join(' ')}" data-proficiency="${item}" data-type="${type}">` +
    ...
    updateSelectionCounters(containers, character) {
        for (const type of this._proficiencyTypes) {
            ...
            const selected = character?.optionalProficiencies?.[type]?.selected?.length || 0;
