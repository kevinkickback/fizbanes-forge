        ...
        for (const entry of entries) {
            ...
            if (typeof entry === 'object') {
                        ...
                        for (const item of entry.items) {
                            if (typeof item === 'string') {
                                ...
                                if (item.name && item.entry) {
                                    const processedName = await textProcessor.processString(item.name);
                                    const processedEntry = await textProcessor.processString(item.entry);
                                    ...
                                    const processedName = await textProcessor.processString(item.name);
                                    const processedEntries = await this._formatFeatureEntries(item.entries);
                ...
                // Handle tables
                    ...
                    if (entry.caption) {
                        const processedCaption = await textProcessor.processString(entry.caption);
                ...
                // Handle Spell Save DC
                    ...
                    if (!character || !abilityAbbr) {
                        ...
                        const profBonus = character.getProficiencyBonus ? character.getProficiencyBonus() : Math.ceil(1 + character.level / 4);
                        const dc = 8 + profBonus + modifier;
                        const processedName = await textProcessor.processString(entry.name || 'Spell Save DC');
                ...
                // Handle Spell Attack Modifier
                    ...
                    if (!character || !abilityAbbr) {
                        ...
                        const processedName = await textProcessor.processString(entry.name || 'Spell Attack Modifier');
