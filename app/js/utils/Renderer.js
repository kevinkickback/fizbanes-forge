/**
 * Renderer.js
 * Main rendering engine based on 5etools architecture
 * Handles recursive rendering of D&D game content with support for nested entries
 * 
 * Usage:
...
class Renderer {
    ...
    _renderInset(entry, textStack, meta) {
        const cssClass = entry.type === "insetReadaloud" ? "rd__b-inset--readaloud" : "";
...
Renderer.spell = class {
    ...
    static getHtmlPtComponents(spell) {
        ...
        const comps = spell.components.map(c => {
            ...
            if (c === "M") return `Material (${spell.materials?.join(", ") || "see text"})`;
            return c;
        }).join(", ");
    ...
    static getHtmlPtDuration(spell) {
        ...
        const dur = Array.isArray(spell.duration) ? spell.duration[0] : spell.duration;
