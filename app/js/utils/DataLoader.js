...
class DataLoader {
    ...
    async loadSpells(source = 'PHB') {
        return this.loadJSON(`${this._baseUrl}spells/spells-${source.toLowerCase()}.json`);
    }
    ...
    async loadClasses(className = 'Fighter') {
        return this.loadJSON(`${this._baseUrl}class/class-${className.toLowerCase()}.json`);
    }
...
DataLoader.loadOptionalFeatures = () => DataLoader.getInstance().loadOptionalFeatures();
DataLoader.loadFluffOptionalFeatures = () => DataLoader.getInstance().loadFluffOptionalFeatures();
DataLoader.loadSources = () => DataLoader.getInstance().loadSources();
DataLoader.loadSubclassSpells = (subclassId) => DataLoader.getInstance().loadSubclassSpells(subclassId);
