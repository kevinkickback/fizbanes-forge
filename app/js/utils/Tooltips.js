...
export class TooltipManager {
    ...
    async showReference(type, name, source, x, y) {
        ...
        const isAlreadyOpen = this._tooltips.some(t => {
            ...
            console.log(`[Tooltip] Comparing "${referenceKey}" with "${existingKey}"`);
            return existingKey === referenceKey;
        });

        if (isAlreadyOpen) {
            console.log(`[Tooltip] Circular reference detected: ${type} - ${name} is already open in the chain, ignoring hover`);
            return;
        }
        ...
        try {
            ...
            this.show(x, y, `<strong>${name}</strong><br><small>Error loading details</small>`);
        }
    }
    ...
    _formatTooltip(data) {
        ...
        if (data.ability || data.size || data.speed) {
            ...
            if (data.ability && Array.isArray(data.ability)) {
                const abilities = data.ability.map(ab => {
                    const abilityStr = Object.entries(ab).map(([key, val]) => {
        ...
        if (data.hd) {
            ...
            if (data.startingProficiencies) {
                ...
                if (data.startingProficiencies.weapons) {
                    ...
                    const weaponsParts = data.startingProficiencies.weapons.map(w => {
                        if (typeof w === "string") {
                            return w;
                        } else if (typeof w === "object" && w.proficiency) {
                            // Handle objects like {proficiency: "firearms", optional: true}
                            return w.optional ? `${w.proficiency} (optional)` : w.proficiency;
                        }
                        return "";
                    }).filter(Boolean);
        ...
        if (data.ability && typeof data.ability === "string" && !data.hd && !data.level) { // Check it's a skill (has ability string but not a spell/class)
            html += '<div class="tooltip-metadata">';
...
export function initializeTooltipListeners(tooltipManager) {
    ...
    function isInTooltipSystem(element) {
        ...
        const inTooltip = element.classList?.contains('tooltip-container') ||
            element.classList?.contains('tooltip') ||
            element.closest('.tooltip-container') !== null;
        const inLink = element.classList?.contains('rd__hover-link') ||
    ...
    document.addEventListener("mouseover", (event) => {
        ...
        if (parentTooltip) {
            ...
            linkTooltipIndex = tooltipManager._tooltips.findIndex(t => t.container === parentTooltip);
        }
        ...
        if (parentTooltip) {
            console.log(`[TooltipSystem] Link is in tooltip at index: ${linkTooltipIndex}`);
        } else {
            console.log(`[TooltipSystem] Link is not in a tooltip (base level)`);
        }

        console.log(`[TooltipSystem] Current stack size: ${tooltipManager._tooltips.length}, keeping up to index: ${keepUpToIndex}`);
        ...
        while (tooltipManager._tooltips.length > keepUpToIndex) {
            ...
            console.log(`[TooltipSystem] Removing tooltip at index ${tooltipManager._tooltips.length}`);
        ...
        if (hoverType && hoverName) {
            ...
            if (inlineContent && (hoverType === 'trait' || hoverType === 'feature')) {
                ...
                tooltipManager.showReference(hoverType, hoverName, hoverSource, event.clientX, event.clientY);
            }

            // Track the newly created tooltip
            if (tooltipManager._tooltips.length > startCount) {
                const newTooltip = tooltipManager._tooltips[tooltipManager._tooltips.length - 1].container;
                activeElements.set(newTooltipDepth, { link, tooltip: newTooltip, timeout: null });
                console.log(`[TooltipSystem] Added tooltip at depth ${newTooltipDepth}, stack size now: ${tooltipManager._tooltips.length}`);
            }
        }
    });
    ...
    document.addEventListener("mouseout", (event) => {
        ...
        if (isInTooltipSystem(fromElement) && isInTooltipSystem(toElement)) {
            ...
            if (fromTooltip && fromTooltip !== toTooltip) {
                const fromIndex = tooltipManager._tooltips.findIndex(t => t.container === fromTooltip);
                const toIndex = toTooltip ? tooltipManager._tooltips.findIndex(t => t.container === toTooltip) : -1;

                // If we're moving to a shallower tooltip (or out of nested tooltip entirely)
                if (fromIndex > toIndex) {
                    console.log(`[TooltipSystem] Moving from tooltip ${fromIndex} to ${toIndex}, removing deeper tooltips`);
