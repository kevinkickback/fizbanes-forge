...
export class TagProcessor {
    ...
    _registerDefaultHandlers() {
        ...
        this.registerHandler("weaponprof", (text) => {
            const parts = TagProcessor.splitTagByPipe(text);
            const displayName = parts.length > 1 ? TagProcessor.escapeHtml(parts[1]) : TagProcessor.escapeHtml(parts[0]);
        ...
        this.registerHandler("5etools", (text) => {
            const parts = TagProcessor.splitTagByPipe(text);
            const displayText = parts.length > 1 ? TagProcessor.escapeHtml(parts[1]) : TagProcessor.escapeHtml(parts[0]);
