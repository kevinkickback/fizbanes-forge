...
export class ProficiencyCore {
    ...
    static removeProficienciesBySource(character, source) {
        ...
        for (const type in character.proficiencySources) {
            ...
            for (const [proficiency, sources] of character.proficiencySources[type].entries()) {
                // Remove this source
    ...
    static getProficienciesWithSources(character, type) {
        ...
        return character.proficiencies[type].map(proficiency => ({
            name: proficiency,
            sources: this.getProficiencySources(character, type, proficiency)
        }));
    }
    ...
    static setOptionalProficiencies(character, type, source, allowed, options) {
        ...
        if (!character.optionalProficiencies[type][sourceLower]) {
            character.optionalProficiencies[type][sourceLower] = { allowed: 0, options: [], selected: [] };
        }
    ...
    static selectOptionalProficiency(character, type, source, proficiency) {
        ...
        this.addProficiency(character, type, proficiency, `${source} Choice`);
    ...
    static deselectOptionalProficiency(character, type, source, proficiency) {
        ...
        this._removeProficiencyFromSource(character, type, proficiency, `${source} Choice`);
    ...
    static getAvailableOptionalProficiencies(character, type, source) {
        ...
        return config.options.filter(option => {
            ...
            const sources = this.getProficiencySources(character, type, option);
            const hasNonChoiceSources = Array.from(sources).some(s => !s.includes('Choice'));
    ...
    static _recalculateOptionalProficiencies(character, type) {
        ...
        for (const source of sources) {
            if (config[source]?.options) {
                config[source].options.forEach(opt => allOptions.add(opt));
            }
        }
        ...
        for (const source of sources) {
            if (config[source]?.selected) {
                config[source].selected.forEach(sel => allSelected.add(sel));
            }
        }
    ...
    static _refundOptionalSkill(character, proficiency, newSource) {
        ...
        for (const source of sources) {
            ...
            const matchingProf = config.selected.find(s =>
                s.toLowerCase().trim() === normalizedProf
            );
            ...
            if (matchingProf) {
                ...
                this._removeProficiencyFromSource(character, 'skills', matchingProf, `${source.charAt(0).toUpperCase() + source.slice(1)} Choice`);
    ...
    static initializeProficiencyStructures(character) {
        ...
        const types = ['skills', 'savingThrows', 'languages', 'tools', 'armor', 'weapons'];
        ...
        if (character.proficiencies.languages.length === 0) {
            this.addProficiency(character, 'languages', 'Common', 'Default');
        }
    }
}
